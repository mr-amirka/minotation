...
| `cnt_→` | `content:"→"` |
| `cnt_` | `content:""` — первое подчёркивание переключает режим |
| `cnt__` | `content:" "` — остальные становятся пробелами |
...
Три случая кавычками не оборачиваются — распознаются сами:

| Токен | CSS | Почему |
|-------|-----|--------|
| `cnt_attr\(href\)` | `content:attr(href)` | значение целиком — вызов функции |
| `cnt--v` | `content:var(--v)` | подстановка |
| `cnt_a_"b"_c` | `content:a "b" c` | кавычка в значении — автор управляет сам |
