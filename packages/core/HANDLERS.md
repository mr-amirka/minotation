...
Две единицы в этой форме недоступны. `%` — потому что тень по грамматике
принимает `<length>`, а не проценты (`bxsh10%` бракуется). `in` (дюймы) — из-за
коллизии с модификатором `in` (inset), который здесь важнее. Обе доступны через
свободную форму: `bxsh_0_0_10in_#000`.
