...
describe('Standard preset — цвета', () => {
  ...
  test('именованный цвет бракуется — цвет задаётся кодом', () => {
    // «пусть принимаются только коды (F00)».
...
describe('Standard preset — box-shadow / text-shadow', () => {
  ...
  test('bxsh19In → брак: заглавный `In` модификатором не является', () => {
    ...
    // С 2026-09-25 неразобранный хвост суффикса теней бракует токен.
...
describe('Standard preset — свойства, добавленные при аудите покрытия', () => {
  ...
  // gapx/gapy — атомарная альтернатива многозначному `gap8_16`.
  test.each([
 ...
 * Аббревиатуры заведены 2026-09-24 («вместо них можно
...
/**
 * Прозрачность переменной и цветовые функции (
