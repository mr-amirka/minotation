...
describe('опции инстанса', () => {
  ...
  /**
   * ПЕРЕСМОТРЕНО: раньше опции читались заново из `mn.options` на каждом
   * `compile` — прямая мутация поля подхватывалась. Проверка показала, что
   * этим нигде не пользуются, поэтому переконфигурация стала явной
   * (`mn.setOptions`), а `mn.options` — только снимок для чтения/отладки.
   */
