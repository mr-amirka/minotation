 ...
 *
 * Раньше сюда проходил ЛЮБОЙ CSS-идентификатор, то есть и все ~150 именованных
 * цветов (`cRed` → `color:red`). Теперь принимаются только коды: `cRed` и `cF00`
 * дают один результат двумя записями, а в CSS попадут оба правила. Побочная польза от закрытого списка: мусор вида
 ...
 *
 * Именованных цветов (`red`, `aliceblue`, …) здесь намеренно нет — принимаются
 * только коды (`F00`). Причина та же, что у остальной канонизации: `cRed`
 * и `cF00` дают один результат двумя записями, и в CSS поедут оба правила.
 * Код ещё и короче.
 *
