 ...
 *
 * `E` — пустая строка, и это НЕ то же, что `none`: `content:""` создаёт
 * псевдоэлемент (обычный приём для `::before`/`::after` с оформлением), а
 ...
 */
  ...
  mn({
    ...
    cnt: (p) => {
      ...
      return styleWrap({
        content: s[0] === '_'
          // Суффикс из одних подчёркиваний схлопывается в пустую строку —
          // подставляем пробел: самый ходовой случай у псевдоэлементов, и за
          // ним оставлена кратчайшая запись `cnt_`.
          ? (v
