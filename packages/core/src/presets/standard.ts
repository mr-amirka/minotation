 ...
 *
 * `S` — пробел (space), мнемоника к `cnt__`; обе записи дают один и тот же
 * CSS. Кавычки двойные, как у всех остальных значений: одиночные в прежнем
 * `content:' '` были разнобоем без причины.
 *
    ...
    cnt: (p) => {
      ...
      return styleWrap({
        content: s[0] === '_'
          ...
          // остальные пробелы. `cnt_` → `""`, `cnt__` → `" "`. Одиночный
          // пробел пишется и мнемоникой `cntS`.
          ? (v
