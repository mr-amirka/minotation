 ...
 * - `+` перед `--` — слагаемое-переменная в `calc` (`w10+--a`, `w10+---safe` для `env`);
 * - `#` перед hex-цветом из 3/4/6/8 цифр — `#` там часть ЗНАЧЕНИЯ, а не id-селектор.
 *
 * `+` перед `--` добавлено 2026-09-25. До этого `+` считался комбинатором соседа
 * всегда, кроме случая «перед цифрой», и `w10+--a` разбирался как
 * `.w10\\+--a+--a{width:10px}`: слагаемое молча терялось, а его остаток уезжал
 * в селектор. В CSS уходило правило, которое не сработает никогда, и БЕЗ
 * предупреждения — запрещено D-004. При этом форма со знаком `-` (`w10---a`)
 * работала, то есть сложение и вычитание были неравноправны без всякой причины,
 * видимой пишущему токен.
 *
 * **Чем платим:** сосед, чьё имя начинается с `--`, перестаёт выражаться через
 * `+` (`p10+--v=10px`). Имена CSS-классов с `--` в начале — вырожденный случай
 * (в самой нотации так пишется только объявление переменной), а вычитание
 * переменной было бы невозможно выразить вообще никак.
 *
