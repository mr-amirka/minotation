...
шаблонные (подстановки `${…}` пропускаются, статические части берутся). Набор суффиксов настраивается:
...
| `mn` | `object` | — | Опции mn-инстанса: `selectorPrefix`, `media`, `strict` (`true` — предупреждения роняют сборку, `MnStrictError`; по умолчанию `false`) |
