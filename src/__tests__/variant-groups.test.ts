...
/* eslint-disable */
/**
 * Комплексные тесты variant groups.
 *
 * Покрывает все сценарии из spec §12 (12-variant-groups-css-output.md).
 * Тесты для `!` модификатора помечены TODO — фича v1-only, не портирована.
 *
 * Мигрировано с v2 API на v1 (createMn/s.arg/mn.setSynonyms/
...
function make(opts) {
  ...
  mn('mt', (p) => ({
    style: {
      marginTop:    p.suffix + 'px',
    },
  }));
  mn('mb', (p) => ({
    style: {
      marginBottom: p.suffix + 'px',
    },
  }));
  // 'c' handler: cF → color:#F, cA → color:#A (suffix = uppercase letter(s))
  mn('c',  (p) => ({
    style: {
      color: '#' + (p.suffix || ''),
    },
...
describe('§2 Базовые случаи variant groups', () => {
  ...
  test('p(10|20|30) → один класс с тремя padding-телами', () => {
    const mn = make();
    mn.getCompiler('class')('p(10|20|30)');
    mn.compile();
    const c = css(mn);
  ...
  test('вложенные (p(10|20)|m5) → один класс с тремя телами', () => {
    const mn = make();
    mn.getCompiler('class')('(p(10|20)|m5)');
    mn.compile();
    const c = css(mn);
...
describe('§3 Multi-attr: один token в разных атрибутах', () => {
  test('p10 через class + m-n + id → три селектора в одном правиле', () => {
    ...
    mn.getCompiler('m-n')('p10');
    mn.getCompiler('id')('p10');
    mn.compile();
...
describe('§4 Состояния + variants', () => {
  test('(p10|m5):hover → псевдокласс в имени и как CSS pseudo', () => {
    const mn = make();
    mn.getCompiler('class')('(p10|m5):hover');
    mn.compile();
    const c = css(mn);
...
describe('§5 Медиа + variants', () => {
  test('(p10|m5)@sm → @media блок с двумя телами', () => {
    ...
    mn.getCompiler('class')('(p10|m5)@sm');
    mn.compile();
    const c = css(mn);
  ...
  test('p(10|20)<.root@sm → media + parent, два тела', () => {
    ...
    mn.getCompiler('class')('p(10|20)<.root@sm');
    mn.compile();
    const c = css(mn);
    expect(c).toContain('@media (max-width:640px)');
...
describe('§6 Родительский контекст + variants', () => {
  test('(p10|m5)<.parent → два тела с parent-контекстом', () => {
    const mn = make();
    mn.getCompiler('class')('(p10|m5)<.parent');
    mn.compile();
    const c = css(mn);
  ...
  test('(p10|m5)<1.clsA → прямой родитель depth=1', () => {
    const mn = make();
    mn.getCompiler('class')('(p10|m5)<1.clsA');
    mn.compile();
    const c = css(mn);
  ...
  test('(p10|m5)<.clsA<.clsB → цепочка двух предков', () => {
    const mn = make();
    mn.getCompiler('class')('(p10|m5)<.clsA<.clsB');
    mn.compile();
    const c = css(mn);
  ...
  test('(p10|m5)<.(clsA.active|clsB):(d|f) — v1: состояние на родителе, дедупликация', () => {
    const mn = make();
    mn.getCompiler('class')('(p10|m5)<.(clsA.active|clsB):(d|f)');
    mn.compile();
    const c = css(mn);
    const token = '.\\(p10\\|m5\\)\\<\\.\\(clsA\\.active\\|clsB\\)\\:\\(d\\|f\\)';
    // v1 поведение: :(d|f) — суффикс на РОДИТЕЛЕ (не на элементе, т.к. стоит после
    // <.(...)), d и f оба → :focus, дедуплицируется до 2 селекторов (не 4)
    expect(c).toContain(`.clsA.active:focus ${token},.clsB:focus ${token}{padding:10px}`);
    expect(c).toContain(`.clsA.active:focus ${token},.clsB:focus ${token}{margin:5px}`);
  });
});
...
describe('§7 Multiplier *N (в конце токена)', () => {
  ...
  test('(p10|m5)<.parent*2 → *N после parent (правильная позиция)', () => {
    const mn = make();
    mn.getCompiler('class')('(p10|m5)<.parent*2');
    mn.compile();
    const c = css(mn);
...
// ─────────────────────────────────────────────────────────────────────────────
// §8 ! модификатор — v1-специфика, не портирована
// Тесты документируют ожидаемый контракт
// ─────────────────────────────────────────────────────────────────────────────

describe('§8 ! модификатор — < направление (v1-only, TODO)', () => {
...
describe('§9 ! модификатор — > направление (v1-only, TODO)', () => {
...
describe('§10 Неизвестный хендлер в variant group → silent skip', () => {
  test('(p10|unknown5) — p10 генерируется, unknown5 не создаёт отдельного CSS', () => {
    const mn = make();
    mn.getCompiler('class')('(p10|unknown5)');
    mn.compile();
...
describe('§11 m-n атрибут + parent', () => {
  test('(p10|m5) через m-n + parent → атрибутный селектор с parent-контекстом', () => {
    const mn = make();
    mn.getCompiler('m-n')('(p10|m5)<.parent');
    mn.compile();
    const c = css(mn);
