...
export default (settings) => {
  ...
  const defaultLoaderSettings = merge([defaultPluginSettingsModule.defaultLoaderSettings, settings.defaultLoaderSettings || {}]);
  const defaultPluginSettings = merge([defaultPluginSettingsModule.defaultPluginSettings, settings.defaultPluginSettings || {}]);
  ...
  return {
    sourceHandle(params) {
      ...
      const settings = merge([defaultLoaderSettings, params.settings || {}]);
      const id = settings.id || '';
      parserProvider(settings.attrs)((scope[id] || (scope[id] = {}))[key] = {},
        params.source || '');
      return {
        source,
      };
      ...
      return {
        startTemplateParse(options) {
          ...
          parseSource(path, {
            ...
            parse,
            each: commonEach,
            exclude: getExclude(settings.exclude,
