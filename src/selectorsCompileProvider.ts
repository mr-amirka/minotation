// @ts-nocheck
/* eslint-disable */
import { aggregate, eachApply, eachTry, escapeCss, extend, flags, forEach, forIn, isArray, joinComma, joinOnly, map, mapEach, mapIn, merge, push, pushArray, reduceEach, reduceIn, routeParseProvider, splitProvider, variants, values } from 'fundamentool';
import { escapeCss, escapeQuote, escapedHalfProvider, escapedSplitProvider, extend, filter, indexOf, joinOnly, map, mapEach, push, pushArray, reduce, repeat, scopeSplit2, slice, unslash, variants } from 'fundamentool';
const variantsBase = require('./mn-utils-shim/variants').base;
...
function getCombinator(name) {
  ...
  return depthMatchs ? [getCombinatorByDepth(parseInt(depthMatchs[1])), depthMatchs[2] || ''] : [' ', name];
}
...
function joinMapsWithFirstValue(
  prefixes, suffixes, separator, end,
) {
  separator = separator || '';
  end = end || '';
...
function provider(instance) {
  ...
  function parseComboName(
    comboName, targetName, multiplierMatch, multiplier,
  ) {
    ...
    return reduce(reduce(variantsBase(comboName), suffixesReduce, {}), (items, essences, suffix) => {
      ...
      const first = getParents(
        childs.shift(), targetName, '',
      );
      return push(items, [essences, map(reduce(
        childs, childsIteratee, first,
      ), mediaFilterIteratee)]);
    }, [],
    );
  }
    ...
    function base(scopes, hasTop) {
      ...
      for (; scopesI < scopesL; scopesI++) {
        ...
        for (; statesI < statesL; statesI++) {
          (matches = regexpScopeSuffix.exec(_state = state = unslash(states[statesI])))
            ? (
              state = matches[1],
              suffix = matches[2]
            )
